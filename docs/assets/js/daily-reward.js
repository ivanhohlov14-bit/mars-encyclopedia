// daily-reward.js — ежедневная награда + рулетка удачи
(function() {
    'use strict';
    ...
    const KINGDOMS = {
        'Аркадия':    { color: '#D4A574', light: '#E8C9A0' },
        'Ксанф':      { color: '#3D3D3D', light: '#6B6B6B' },
        'Эдем':       { color: '#F4A460', light: '#F7C98A' },
        'Эридания':   { color: '#F5D76E', light: '#FAE9A0' },
        'Кхонг':      { color: '#A9A9A9', light: '#C8C8C8' },
        'Авсония':    { color: '#87CEEB', light: '#B0D8EB' },
        'Кимерия':    { color: '#B19CD9', light: '#D1C4E9' },
        'Серпентида': { color: '#E57373', light: '#F5A0A0' },
        'Эритрей':    { color: '#64B5F6', light: '#90CAF9' },
        'Утопия':     { color: '#4DD0E1', light: '#80DEEA' },
        'Эллада':     { color: '#FF8A65', light: '#FFAB91' },
        'Аливасото':  { color: '#81C784', light: '#A5D6A7' }
    };

    // Награды за дни стрика
    const STREAK_REWARDS = {
        1: 5, 2: 10, 3: 15, 4: 20, 5: 25, 6: 35, 7: 50,  // 7-й день — бонус
    ...
    // ============================================================
    // Утилита: получить сегодняшнюю дату
    // ============================================================
    function todayStr() {
    ...
    // ============================================================
    function showDailyReward(streak, rewardXP, isNewStreak) {
        const kingdom = getCurrentKingdom();
        const nextReward = STREAK_REWARDS[streak + 1] || (streak + 1) * 5;
    ...
    async function init() {
        ...
        // Если уже заходил сегодня — ничего не показываем
        if (lastDate === today) {
            // Проверяем, не крутил ли рулетку
            const rouletteKey = `roulette_${currentUser.id}_${today}`;
            if (!localStorage.getItem(rouletteKey)) {
                // Не крутил — покажем плашку внизу
                setTimeout(() => {
        ...
        if (lastDate) {
            ...
            // Если пропустил больше 1 дня — стрик сбрасывается на 1
        }
        ...
        // Показываем модалку через 1 сек после загрузки
        setTimeout(() => {
            showDailyReward(newStreak, rewardXP, true);
        }, 1000);
    }
