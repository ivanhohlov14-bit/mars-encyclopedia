...
(function() {
  ...
  function ensureAudioContext() {
   ...
   // ============================================================
  // 5. ВОСПРОИЗВЕДЕНИЕ ОДНОЙ НОТЫ (с сэмплом, фильтром и реверберацией)
  // ============================================================
  function playNote(ctx, note, startTime, durationSec) {
    // Создаём выходной узел
    ...
    if (gusliBuffer) {
      ...
      const baseFreq = 261.63; // предположим, сэмпл на C4
      const playbackRate = note.freq / baseFreq;
      source.playbackRate.value = playbackRate;

      // Огибающая (щипок + затухание)
      const env = ctx.createGain();
      env.gain.setValueAtTime(0.001, startTime);
      env.gain.linearRampToValueAtTime(0.3, startTime + 0.015);
      env.gain.exponentialRampToValueAtTime(0.001, startTime + durationSec * 0.8);

      // Фильтр для тепла (убираем резкость)
      const filter = ctx.createBiquadFilter();
      filter.type = 'lowpass';
      filter.frequency.value = 2000;
      filter.Q.value = 1.0;

      // --- Простая стерео-реверберация через задержки ---
      ...
      feedbackGain.gain.value = 0.4; // сила отражений

      // Сухой и мокрый сигналы
      const dry = ctx.createGain();
      dry.gain.value = 0.7;
      const wet = ctx.createGain();
      wet.gain.value = 0.3;

      // Подключение: источник -> огибающая -> фильтр -> сухой
      source.connect(env);
      env.connect(filter);
      filter.connect(dry);
      dry.connect(output);

      // Эффект: фильтр -> задержки -> feedback -> задержки -> мокрый
    ...
    // --- 2. Fallback: синтез осциллятором (с фильтром) ---
    else {
